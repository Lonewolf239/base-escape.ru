document.addEventListener("DOMContentLoaded", () => {
    ...
    sun.addEventListener("mousedown", (e) => {
        if (e.button !== 0 || state === "exploded" || state === "irreversible")
            return;
    ...
    const releaseSun = () => {
        if (state === "holding") state = "recovering";
    };
    ...
    function triggerCinematicExplosion() {
        ...
        function renderExplosionFrame(timestamp) {
            ...
            if (explosionTime < PHASE_1_END) {
                ...
                const t =
                    (explosionTime - PHASE_1_END) / (PHASE_2_END - PHASE_1_END);
            ...
            if (
                explosionTime > 900 &&
                !document.body.classList.contains("glitch-stage-1")
            )
                document.body.classList.add("glitch-stage-1");
            if (
                explosionTime > 2200 &&
                !document.body.classList.contains("glitch-stage-2")
            )
                document.body.classList.add("glitch-stage-2");
            if (
                explosionTime > 3600 &&
                !document.body.classList.contains("glitch-stage-3")
            )
            ...
            for (let i = solidChunks.length - 1; i >= 0; i--) {
                ...
                if (
                    projX < -margin ||
                    projX > canvas.width + margin ||
                    projY < -margin ||
                    projY > canvas.height + margin ||
                    chunk.z <= -FOV
                ) {
    ...
    function triggerTerminalCrash() {
        ...
        document.body.style.cssText =
            "background: #000; background-color: #000; margin: 0; padding: 0; overflow: hidden; width: 100vw; height: 100vh; cursor: none;";
        document.body.innerHTML = "";

        const crtContainer = document.createElement("div");
        crtContainer.style.cssText =
            "position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background-color: #030305; z-index: 999998; transform-origin: center center; overflow: hidden;";
        ...
        function typeCrashLine() {
            if (lineIndex < crashLogs.length) {
                ...
                if (text.startsWith("guest")) line.className += " prompt-line";
                else if (
                    text.includes("[   ERROR    ]") ||
                    text.includes("FATAL")
                ) {
        ...
        function initializeInteractiveInput() {
            ...
            inputContainer.className =
                "terminal-line prompt-line active-input-line";
            inputContainer.innerHTML =
                'guest@universe:~# <span class="user-typed-text"></span><span class="terminal-cursor">█</span>';
                ...
                if (e.key === "Enter") {
                    ...
                    if (trimmedCmd === "42") {
                        let currentCount = parseInt(
                            localStorage.getItem("supernova_42_count") || "0",
                            10,
                        );
                        currentCount++;
                        ...
                        if (currentCount >= 42) {
                            ...
                            logLine.textContent = `bash: ${currentCount}`;
        ...
        function executeCrtScreenOn() {
            ...
            function streamBootLogs() {
                if (bootIndex < bootLogs.length) {
                    ...
                    if (
                        currentLog.includes("[ OK ]") ||
                        currentLog.includes("successful")
                    ) {
                        line.style.color = "#00ff66";
                        line.style.textShadow =
                            "0 0 8px rgba(0, 255, 102, 0.7)";
                    } else if (currentLog.startsWith("["))
                    ...
                    if (
                        currentLog.includes("EXT4-fs") ||
                        currentLog.includes("timeline threads")
                    )
                        delay = 700 + Math.random() * 600;
                    else if (currentLog.includes("[ OK ]"))
    ...
    function initUltimateSecret() {
        const triggerApocalypse = (e) => {
            ...
            window.removeEventListener("click", triggerApocalypse, {
                capture: true,
            });
            window.removeEventListener("touchstart", triggerApocalypse, {
                capture: true,
            });
            ...
            const rolesList = [
                ...
                {
                    p: "Главный архитектор систем (Chief Architect)",
                    n: "Lonewolf239",
                },
                ...
                {
                    p: "Тот самый чувак, исправивший баги за стажером",
                    n: "Lonewolf239",
                },
                { p: "Главный UI/UX дизайнер интерфейса", n: "Lonewolf239" },
                {
                    p: "Исследователь пользовательского опыта (UX Researcher)",
                    n: "Lonewolf239",
                },
                ...
                {
                    p: "Специалист по анимациям CSS3 и Keyframes",
                    n: "Lonewolf239",
                },
                {
                    p: "Инженер эффектов симуляции CRT-монитора",
                    n: "Lonewolf239",
                },
                {
                    p: "Ведущий системный администратор terminal",
                    n: "Lonewolf239",
                },
                {
                    p: "Администратор баз данных LocalStorage",
                    n: "Lonewolf239",
                },
                {
                    p: "DevOps Engineer & Infrastructure Lead",
                    n: "Lonewolf239",
                },
                {
                    p: "Специалист по падениям CI/CD пайплайнов",
                    n: "Lonewolf239",
                },
                {
                    p: "Директор Департамента обеспечения качества (QA Director)",
                    n: "Lonewolf239",
                },
                {
                    p: "Ведущий инженер по автоматизации тестирования",
                    n: "Lonewolf239",
                },
                {
                    p: "Специалист по ручному тестированию (Manual QA)",
                    n: "Lonewolf239",
                },
                {
                    p: "Тестировщик под высокими нагрузками (Stress Tester)",
                    n: "Lonewolf239",
                },
                {
                    p: "Звукорежиссер перезаписи и монтажа звука",
                    n: "Lonewolf239",
                },
                {
                    p: "Музыкальный супервайзер (Music Supervisor)",
                    n: "Lonewolf239",
                },
                {
                    p: "Инженер по аудио-компрессии кодека .m4a",
                    n: "Lonewolf239",
                },
                { p: "Скаут локаций (Location Scout)", n: "Lonewolf239" },
                {
                    p: "Постановщик трюков (Stunt Coordinator)",
                    n: "Lonewolf239",
                },
                {
                    p: "Исполнитель опасных трюков (Каскадер)",
                    n: "Lonewolf239",
                },
                { p: "Ассистент каскадера", n: "Lonewolf239" },
                {
                    p: "Постановщик трюка «Медитация на разделительной полосе»",
                    n: "Lonewolf239",
                },
                { p: "Художник по костюмам", n: "Lonewolf239" },
                {
                    p: "Менеджер по реквизиту (Property Master)",
                    n: "Lonewolf239",
                },
                {
                    p: "Директор по освещению кадра (Солнечный закат)",
                    n: "Lonewolf239",
                },
                {
                    p: "Колорист финального изображения (Color Grader)",
                    n: "Lonewolf239",
                },
                {
                    p: "Директор по MARKETING и связям с общественностью",
                    n: "Lonewolf239",
                },
                {
                    p: "Копирайтер терминальных логов и аварийных отчетов",
                    n: "Lonewolf239",
                },
                {
                    p: "Главный специалист по решению Git-конфликтов",
                    n: "Lonewolf239",
                },
                {
                    p: "Сортировщик и чистильщик мертвого кода",
                    n: "Lonewolf239",
                },
                {
                    p: "Менеджер по доставке кофе и энергетиков (Catering)",
                    n: "Lonewolf239",
                },
                {
                    p: "Шеф-повар полевой кухни для одного разработчика",
                    n: "Lonewolf239",
                },
                ...
                {
                    p: "Психологическая поддержка команды разработки",
                    n: "Lonewolf239",
                },
                {
                    p: "Тот, кто загорелся идеей и набросал базу за пару дней",
                    n: "Lonewolf239",
                },
                {
                    p: "Тот, кто забил болт на код на несколько месяцев",
                    n: "Lonewolf239",
                },
                {
                    p: "Тот, кто собрал всё в монолит за сутки перед деплоем",
                    n: "Lonewolf239",
                },
                { p: "Chief Executive Officer (CEO)", n: "Lonewolf239" },
                { p: "Chief Technology Officer (CTO)", n: "Lonewolf239" },
                {
                    p: "Chief Cleaning Officer (Уборщик console.log)",
                    n: "Lonewolf239",
                },
                { p: "Философский консультант по числу 42", n: "Дуглас Адамс" },
                {
                    p: "Особая благодарность за саундтрек молодости",
                    n: "Честер Беннингтон (Linkin Park)",
                },
                {
                    p: "Вдохновитель масштаба взрывов и общего пафоса",
                    n: "Майкл Бэй",
                },
                {
                    p: "Технический спонсор проекта",
                    n: "Кофеин, Бессонница и Чистый Энтузиазм",
                },
            ];
                ...
                timer42 = setTimeout(() => {
                    if (!isFailed) {
                        ...
                        window.location.href =
                            "/42/42/42/42/42/42/42/42/42/42/42/index.html";
                    }
                }, waitTime);
            }, creditsDuration);
        };

        window.addEventListener("click", triggerApocalypse, {
            once: true,
            capture: true,
        });
        window.addEventListener("touchstart", triggerApocalypse, {
            once: true,
            capture: true,
        });
    }
