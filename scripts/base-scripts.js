...
function showAdminLogin() {
    ...
    const attemptLogin = async () => {
        ...
        try {
            ...
            if (data.status === "ok") window.location.href = "/admin.php";
            else {
            ...
            submit.textContent = "NETWORK ERROR";
        }
    };
...
function setupMobileMenu() {
    ...
    function restoreDesktop() {
        const allButtons = Array.from(nav.querySelectorAll("button")).filter(
            (btn) => !btn.classList.contains("menu-toggle-btn"),
        );
...
function createScrollTopButton(useNativeSmooth = true) {
    ...
    scrollTopBtn.addEventListener("click", () => {
        const targets = scrollContainers
            .map((el) => {
                if (!el) return null;
...
function createComet() {
    ...
    if (Math.abs(deltaX) < 30 && Math.abs(deltaY) < 30) {
        if (Math.abs(deltaX) < 30)
            deltaX =
                deltaX >= 0
                    ? Math.min(50, maxDeltaX)
                    : Math.max(-50, maxDeltaX);
        if (Math.abs(deltaY) < 30)
            deltaY =
                deltaY >= 0
                    ? Math.min(100, maxDeltaY)
                    : Math.max(-100, maxDeltaY);
    }
...
function loadContent() {
    ...
    if (footer) {
        ...
        async function addLastUpdate() {
            try {
                const githubPath =
                    "https://api.github.com/repos/Lonewolf239/base-escape.ru/commits?per_page=1";
                ...
                if (
                    htmlLang &&
                    ["ru", "en", "de"].includes(htmlLang.split("-")[0])
                )
                ...
                switch (currentLang) {
                    case "ru":
                        updateText = "Последнее обновление:";
                        break;
                    case "de":
                        updateText = "Letzte Aktualisierung:";
                        break;
                    default:
